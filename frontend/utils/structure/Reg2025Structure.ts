...
export default class Reg2025Structure extends Structure {
    public isThereDay2() {
    ...
    public isPlayerInDay2(points: number) {
        return this.isThereDay2() && points >= match(this.tournamentPlayers)
            ...
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 65 && tournamentPlayers <= 128), () => 12)
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 129 && tournamentPlayers <= 256), () => 15)
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 257 && tournamentPlayers <= 512), () => 18)
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 513 && tournamentPlayers <= 1024), () => 18)
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 1025 && tournamentPlayers <= 2048), () => 18)
            .with(P.when((tournamentPlayers) => tournamentPlayers >= 2049 && tournamentPlayers <= 4096), () => 21)
            .otherwise(() => 21)
    }
}